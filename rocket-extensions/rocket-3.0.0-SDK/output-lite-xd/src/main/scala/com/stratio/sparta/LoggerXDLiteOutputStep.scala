...
class LoggerXDLiteOutputStep(
  ...
  override def save(data: DataFrame, outputOptions: OutputOptions): Unit = {
    ...
    if (metadataEnabled){
      val info1 = s"Table name: $tableName"
      logger.info(info1)
      // · Reporting messages
      reportInfoLog(phase="init", msg = info1)

      val info2 = s"Save mode is set to ${outputOptions.saveMode}"
      logger.info(info2)
      // · Reporting messages
      reportInfoLog(phase="init", msg = info2)
    }
    data.foreach{ row =>
